...
__markInstalledToolAsUninstalled() {
    ...
    AbkLib_PrintTrace $TRACE_DEBUG "LCL_INSTALLED_TYPE = $LCL_INSTALLED_TYPE"
    AbkLib_PrintTrace $TRACE_DEBUG "LCL_INSTALLED_ITEM = $LCL_INSTALLED_ITEM"
    ...
    [ $? -eq 0 ] && AbkLib_PrintTrace $TRACE_DEBUG "$LCL_INSTALLED_TYPE/$LCL_INSTALLED_ITEM removed successfully."
    ...
    if jq -e 'length == 0' "$INSTALLED_DIR/$INSTALLED_FILE" | grep -q true; then
        ...
        [ $? -eq 0 ] && AbkLib_PrintTrace $TRACE_DEBUG "File is empty. Deleted."
    fi
...
__uninstallItem() {
    ...
    AbkLib_PrintTrace $TRACE_INFO "${YLW}[$LCL_APP uninstalling ...]${NC}"
    local TOOL_INSTALLED=$TRUE

    while IFS= read -r UNINSTALL_STEP; do
        AbkLib_PrintTrace $TRACE_INFO "uninstall step: ${YLW}$UNINSTALL_STEP${NC}"
...
__uninstallItemList() {
    ...
    if [ -e "$INSTALLED_DIR/$INSTALLED_FILE" ]; then
        ...
        AbkLib_PrintTrace $TRACE_DEBUG "${ORG}Installed type: $LCL_INSTALLED_TYPE: ${LCL_INSTALLED_ITEM_LIST[@]}${NC}"
    fi
    ...
    if [ "$LCL_INSTALLED_ITEM_LIST" != "null" ] && [ "$LCL_INSTALLED_ITEM_LIST" != "" ]; then
        AbkLib_PrintTrace $TRACE_INFO "${ORG}Uninstalling previously installed type: ${LCL_INSTALLED_ITEM_LIST[@]}${NC}"
        ...
        while IFS= read -r LCL_INSTALLED_ITEM; do
            ...
            AbkLib_PrintTrace $TRACE_INFO "uninstall step list: ${YLW}$UNINSTALL_STEPS${NC}"
            __uninstallItem "$LCL_INSTALLED_TYPE" "$LCL_INSTALLED_ITEM" "$UNINSTALL_STEPS" \
                ...
                || AbkLib_PrintTrace $TRACE_ERROR "${RED}ERROR: $LCL_INSTALLED_ITEM uninstall failed${NC}"
        done <<< "$LCL_INSTALLED_ITEM_LIST"
    else
        AbkLib_PrintTrace $TRACE_DEBUG "No installed $LCL_INSTALLED_TYPE found."
    fi
...
__deleteBinDirLink() {
    ...
    if [ -d $HOME_BIN_DIR ]; then
        AbkLib_PrintTrace $TRACE_INFO "[Deleting directory link $HOME_BIN_DIR to $LCL_CURRENT_DIR/$SH_BIN_DIR ...]"
...
__uninstallRequiredTools() {
    ...
    if [ "$LCL_INSTALLED_ITEM_LIST" != "null" ] && [ "$LCL_INSTALLED_ITEM_LIST" != "" ]; then
        AbkLib_PrintTrace $TRACE_INFO "${ORG}Uninstalling jq: ${LCL_INSTALLED_ITEM_LIST[@]}${NC}"
...
uninstall_abkEnv_main() {
    ...
    __deleteBinDirLink || AbkLib_PrintTrace $TRACE_ERROR "${RED}ERROR:${NC} __deleteBinDirLink failed with $?"
