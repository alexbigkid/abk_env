...
__updatePackageManager() {
    ...
    if [ "$LCL_UPDATE_PACKAGES" != "" ]; then
        AbkLib_PrintTrace $TRACE_INFO "${ORG}[Updating packages]${NC}"
        while IFS= read -r LCL_UPDATE_PACKAGE; do
            AbkLib_PrintTrace $TRACE_DEBUG "${ORG}$LCL_UPDATE_PACKAGE${NC}"
            # eval $LCL_UPDATE_PACKAGE
        done <<< "$LCL_UPDATE_PACKAGES"
    else
        AbkLib_PrintTrace $TRACE_CRITICAL "Did NOT find the update_packages section"
    fi
...
__installItem() {
    ...
    AbkLib_PrintTrace $TRACE_DEBUG "LCL_ITEM                    = $LCL_ITEM"
    AbkLib_PrintTrace $TRACE_DEBUG "LCL_INSTALL_INSTRACTIONS    = $LCL_INSTALL_INSTRACTIONS"
    AbkLib_PrintTrace $TRACE_DEBUG "LCL_INSTALL_TYPE            = $LCL_INSTALL_TYPE"
    AbkLib_PrintTrace $TRACE_DEBUG "LCL_CHECK_INSTRACTIONS      = $LCL_CHECK_INSTRACTIONS"

    while IFS= read -r LCL_CHECK_STEP; do
        AbkLib_PrintTrace $TRACE_DEBUG "LCL_CHECK_STEP = $LCL_CHECK_STEP"
        EVAL_CHECK_STEP=$(eval $LCL_CHECK_STEP 2>/dev/null)
        AbkLib_PrintTrace $TRACE_DEBUG "EVAL_CHECK_STEP = $EVAL_CHECK_STEP"
        if [ "$EVAL_CHECK_STEP" != "" ]; then
            LCL_IS_APP_INSTALLED=0
        fi
        AbkLib_PrintTrace $TRACE_DEBUG "LCL_IS_APP_INSTALLED = $LCL_IS_APP_INSTALLED"
    done <<< "$LCL_CHECK_INSTRACTIONS"

    if [ $LCL_IS_APP_INSTALLED -eq 1 ]; then
        AbkLib_PrintTrace $TRACE_INFO "${YLW}[$LCL_ITEM installing ...]${NC}"
        ...
        AbkLib_PrintTrace $TRACE_INFO "${GRN}$LCL_ITEM already installed${NC}"
    fi
...
__installItemList() {
   ...
   # install items
    ...
    if [ "$LCL_INSTALL_ITEMS" != "" ]; then
        ...
        AbkLib_PrintTrace $TRACE_INFO "${ORG}[Installing $LCL_ITEM: ${INSTALL_ITEM_LIST[@]}]${NC}"
        for INSTALL_ITEM in "${INSTALL_ITEM_LIST[@]}"; do
            AbkLib_PrintTrace $TRACE_DEBUG "${ORG}$INSTALL_ITEM${NC}"
            INSTALL_STEPS=$(echo $LCL_INSTALL_ITEMS | jq -r ".\"$INSTALL_ITEM\"[]")
            AbkLib_PrintTrace $TRACE_DEBUG "${ORG}INSTALL_STEPS = $INSTALL_STEPS${NC}"
            CHECK_STEP=$(echo $LCL_CHECK_ITEMS | jq -r ".\"$INSTALL_ITEM\"[]")
            AbkLib_PrintTrace $TRACE_DEBUG "${ORG}CHECK_STEP = $CHECK_STEP${NC}"
            __installItem "$INSTALL_ITEM" "$INSTALL_STEPS" "$LCL_INSTALLED_TYPE" "$CHECK_STEP" || AbkLib_PrintTrace $TRACE_ERROR "${RED}ERROR: $INSTALL_ITEM installation failed${NC}"
        done
    fi
...
__createBinDirLink() {
    ...
    AbkLib_PrintTrace $TRACE_DEBUG "LCL_CURRENT_DIR = $LCL_CURRENT_DIR"
    if [ ! -d $HOME_BIN_DIR ]; then
        AbkLib_PrintTrace $TRACE_INFO "[Creating $HOME_BIN_DIR directory link to $LCL_CURRENT_DIR/$SH_BIN_DIR ...]"
...
install_abkEnv_main() {
    ...
    AbkLib_PrintTrace $TRACE_DEBUG "MAIN_TOOLS_JSON = $MAIN_TOOLS_JSON"
    ...
    AbkLib_PrintTrace $TRACE_INFO "${ORG}[$LCL_DESCRIPTION]${NC}"
    ...
    __createBinDirLink || AbkLib_PrintTrace $TRACE_ERROR "${RED}ERROR:${NC} __createBinDirLink failed with $?"
